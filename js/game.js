...
function position_map() {
	...
	if (change && window.wtile) {
		...
		wtile.x =
			ceil(wtile.x / (textures[wtile_name][0].width / 1.0)) * (textures[wtile_name][0].width / 1.0) -
			textures[wtile_name][0].width / 1.0;
		wtile.y =
			ceil(wtile.y / (textures[wtile_name][0].height / 1.0)) * (textures[wtile_name][0].height / 1.0) -
			textures[wtile_name][0].height / 1.0;
	}
	if (character) {
		if (manual_centering)
			(character.x = c_round(width / 2.0 + ch_disp_x)), (character.y = c_round(height / 2.0 + ch_disp_y));
...
function handle_entities(data, args) {
	...
	for (var i = 0; i < data.monsters.length; i++) {
		var old_events =
			future_entities.players[data.monsters[i].id] && future_entities.players[data.monsters[i].id].events;
		future_entities.monsters[data.monsters[i].id] = data.monsters[i];
		if (old_events)
			future_entities.monsters[data.monsters[i].id].events =
				old_events + future_entities.monsters[data.monsters[i].id].events;
	}
}
...
function draw_entities() {
	for (entity in entities) {
		...
		if (
			...
			(!character &&
				!within_xy_range(
					{ map: current_map, in: current_in, vision: [700, 500], x: map.real_x, y: map.real_y },
					current,
				))
		) {
...
function sync_entity(current, monster) {
	...
	if (current.resync) {
		...
		if (monster.moving)
			(current.engaged_move = -1),
				(current.move_num = 0); // this was only current.move_num=0 before, improved it for "current.move_num!=current.engaged_move"
		else
			(current.engaged_move = current.move_num = monster.move_num),
				(current.angle = (monster.angle === undefined && 90) || monster.angle),
	...
	if (current.move_num != current.engaged_move) {
		...
		speedm =
			simple_distance({ x: current.real_x, y: current.real_y }, { x: monster.going_x, y: monster.going_y }) /
			(simple_distance(monster, { x: monster.going_x, y: monster.going_y }) + EPS);
...
function update_overlays() {
	...
	if (character) {
		if (!cached("att", character.attack))
			$(".attackui").html(
				((character.ctype == "priest" && "HEAL ") || "ATT ") +
					((character.ctype == "priest" && character.heal) || character.attack),
			);
		...
		if (inventory && !cached("igold", character.gold))
			$(".goldnum").html(
				to_pretty_num(character.gold + (new Date().getDate() == 101 && new Date().getMonth() == 3 ? 1014201800 : 0)),
			);
...
function the_game(demo) {
	...
	if (bowser.mac && bowser.firefox && !engine_mode)
		renderer = new PIXI.CanvasRenderer(width, height, {
			antialias: antialias,
			transparent: false,
		}); //, resolution:window.devicePixelRatio etc. doesn't work [15/11/16]
	else if (retina_mode && !engine_mode)
...
function demo_entity_logic(sprite) {
	...
	if (
		sprite.boundary &&
		(sprite.going_x < sprite.boundary[0] ||
			sprite.going_x > sprite.boundary[2] ||
			sprite.going_y < sprite.boundary[1] ||
			sprite.going_y > sprite.boundary[3])
	);
...
function init_socket(args) {
	...
	if (
		...
		(Cookies.get("windows") ||
			...
			window.location.host == "advanture.land" ||
			window.location.host == "x.thegame.com")
	)
		server_addr = "127.0.0.1"; // Cookies.set('windows','1',{expires:12*365});
	else if (is_sdk) {
		if (window.location.origin == "http://127.0.0.1/") server_addr = "127.0.0.1";
		else server_addr = "127.0.0.1";
	}
	...
	socket.on("start", function (data) {
		...
		if (!no_html) {
			...
			$("body").append(
				'<input id="chatinput" onkeypress="if(event.keyCode==13) say($(this).rfval()); send_typing();" type="text" autocomplete="off" name="alchatinput" placeholder=""/>',
			);
		}
		...
		try {
			...
			if (data) {
				...
				handle_information([
					{ type: "code", code: code, run: (explicit_slot && true) || false, slot: code_slot, reset: true },
				]);
			}
		} catch (e) {
			console.log(e);
		...
		draw_trigger(function () {
			if (response == "elixir") {
				...
				render_interaction({
					auto: true,
					skin: "lionsuit",
					message: "You can't change your home server for another " + to_pretty_float(data.hours) + " hours!",
				});
				...
				if (data.goblin)
					render_interaction({
						auto: true,
						skin: G.npcs.lostandfound.skin,
						message: "Ugh, you're sick! Come back when you are healed!",
					});
			...
			else if (response == "need_auth")
				ui_log("To perform this action your account needs a game client authorization", "gray");
	...
	socket.on("emotion", function (data) {
		draw_trigger(function () {
			...
			if (emotion == "drop_egg") {
				if (player)
					map_animation(
						random_one(["egg0", "egg1", "egg2", "egg3", "egg4", "egg5", "egg6", "egg7", "egg8", "goldenegg"]),
						{
							x: get_x(player),
							y: get_y(player) + 1,
	...
	socket.on("ui", function (data) {
		...
		if (data.cevent && data.name == character.name)
			call_code_function("trigger_character_event", (data.cevent === true && data.type) || data.cevent, data);
		// show_json(data);
		draw_trigger(function () {
			if (in_arr(data.type, ["+$", "-$"])) {
				...
				if (seller.me)
					call_code_function("trigger_event", "sale", {
						buyer: data.buyer,
						item: data.item,
						num: data.num,
						slot: data.slot,
				...
				if (sender) {
					map_animation("gold", {
						x: get_x(sender),
						y: get_y(sender) - 36,
						target: { x: get_x(sender), y: get_y(sender) - 90, height: 0, fade: true },
					});
				...
				data.ids.forEach(function (id) {
					...
					if (attacker)
						disappearing_clone(attacker, {
							x: (get_x(entity) + get_x(attacker) * 2) / 3.0,
							y: (get_y(entity) + get_y(attacker) * 2) / 3.0,
							random: true,
						});
				});
			...
			} else if (is_sdk) console.log("Unhandled 'ui': " + data.type);
		});
	});
	...
	socket.on("action", function (data) {
		...
		if (!attacker) return;
		if (!target)
			(target = { x: data.x, y: data.y, map: attacker.map, in: attacker["in"], height: 0, width: 0, m: data.m }),
				(no_target = true);
	...
	socket.on("hit", function (data) {
		...
		if (data.stacked && entity && entity.me)
			call_code_function("trigger_character_event", "stacked", { method: "attack", ids: data.stacked });
		if (data.mobbing && entity && entity.me)
			call_code_function("trigger_character_event", "mobbing", { intensity: data.mobbing });
		...
		draw_trigger(function () {
			...
			if (entity && data.projectile && !evade && G.projectiles[data.projectile].hit_text) {
				d_text(G.projectiles[data.projectile].hit_text[0], entity, {
					color: G.projectiles[data.projectile].hit_text[1],
					size: "huge",
					offset: -offsets * 25 - 10,
				});
				offsets += 1;
			}
			if (
				...
				data.projectile &&
				!evade &&
				G.projectiles[data.projectile].kill_text &&
				(entity.dead || entity.hp <= 0)
			) {
				d_text(G.projectiles[data.projectile].kill_text[0], entity, {
					color: G.projectiles[data.projectile].kill_text[1],
					size: "huge",
					offset: -offsets * 25 - 10,
				});
				offsets += 1;
	...
	socket.on("trade_history", function (data) {
		...
		data.forEach(function (h) {
			...
			if (h[0] == "buy") {
				html +=
					"<div>- Bought " + prefix + "'" + item + "' from " + h[1] + " for " + to_pretty_num(h[3]) + " gold</div>";
		...
		if (character.home != server_region + server_identifier) {
			render_interaction({
				...
				message:
					...
					character.home +
					". Would you like to set this server as your home?",
				button: "Yes!",
				onclick: function () {
			...
			render_interaction({
				...
				skin: "lionsuit",
				message:
					"This is your home server. Make sure to follow daily and nightly server events! Just click the time icon in the bottom left corner for the schedule!",
			});
		}
	}
			...
			render_interaction(
				{
					auto: true,
					skin: "brewingwitch",
					message: "My child, bring me the materials I seek and I shall unlock unimaginable horrors for you!",
				},
				"return_html",
			),
		);
...
function map_click(event) {
	...
	if (event && event.data && event.data.global) {
		...
		if ((blink_pressed || mssince(last_blink_pressed) < 360) && character.ctype == "mage") {
			...
			socket.emit("skill", {
				name: "blink",
				x: character.real_x + dx,
				y: character.real_y + dy,
...
function update_sprite(sprite) {
	...
	if (sprite.stype == "full") {
		...
		if (
			sprite.walking &&
			ms_check(sprite, "walk", base_ms - ((sprite.speed + ((sprite.fx && sprite.fx.aaa && 500) || 0)) / 2 || 0))
		)
			sprite.walking++; //sprite.updates%20==1
		...
		else if (sprite.last_stop && mssince(sprite.last_stop) < ((sprite.s && sprite.s.dash && 5) || 180))
			i = sequence[sprite.last_walking % sequence.length];

		if ((sprite.type == "character" || sprite.humanoid) && (i === 0 || i === 2) && original != i)
			sfx("walk", sprite.real_x, sprite.real_y);
		...
		if (
			sprite.s &&
			(sprite.s.charging || sprite.s.dash) &&
			ms_check(sprite, "clone", (sprite.s.charging && 80) || 30)
		) {
	...
	if (sprite.mtype && !(no_graphics || paused)) {
		if (sprite.mtype == "dice") {
			if (sprite.shuffling) {
				...
				if (
					(!sprite.locked && sprite.shuffling && !(sprite.updates % 40)) ||
					(sprite.cskin != "0" && sprite.cskin != "1")
				) {
...
function add_monster(data) {
	...
	if (def.size)
		(sprite.height *= def.size), (sprite.width *= def.size), (sprite.mscale = def.size), (sprite.hpbar_wdisp = -5);
...
function player_effects_logic(sprite) {
	...
	if (sprite.s.phasedout && !sprite.fx.phs) {
		...
		if (sprite.me && 0) {
			stage.cfilter_rblur = new PIXI.filters.RadialBlurFilter(
				random_one([-0.5, -0.25, 0.25, 0.5]),
				[width / 2, height / 2],
				3,
				-1,
			);
...
function effects_logic(sprite) {
	...
	if (sprite.s.filter && !sprite["filter_" + sprite.s.filter.name]) {
		...
		if (sprite.s.filter.scale)
			(sprite.scale = new PIXI.Point(sprite.s.filter.scale, sprite.s.filter.scale)),
				(sprite.mscale = sprite.s.filter.scale);
	}
	...
	if (
		sprite.type == "monster" &&
		!Object.keys(sprite.fx).length &&
		sprite.real_alpha < 1 &&
		!sprite.dead &&
		!(sprite.appearing || sprite.disappearing || sprite.fading_out)
	) {
...
function cosmetics_logic(sprite) {
	...
	needed.forEach(function (cid) {
		...
		if (c.stype == "copy") {
			...
			c.x = xx + x_disp;
			c.y = -4 + yy;
			if (pickaxe && sprite.fx && sprite.fx.attack && sprite.fx.attack[1] == 8)
				assassin_smoke(get_x(sprite) + ((sprite.j == 1 && -10) || (sprite.j == 2 && 10) || 0), get_y(sprite) - 8);
		}
		if ((last_cx_d[0] || last_cx_d[1]) && !c.moved && c.skin == last_cx_name)
			(c.moved = true), (c.x += last_cx_d[0]), (c.y += last_cx_d[1]);
	});
...
function add_door(door) {
	...
	function door_right_click(event) {
		...
		if (is_electron && electron_data.platform == "mas" && door[4] == "tavern")
		...
		if (
			!is_door_close(character.map, door, character.real_x, character.real_y) ||
			!can_use_door(character.map, door, character.real_x, character.real_y)
		) {
...
function add_quirk(quirk) {
	...
	if (is_mobile || in_arr(quirk[4], ["upgrade", "compound", "invisible_statue"]))
		sprite.on("mousedown", quirk_right_click).on("touchstart", quirk_right_click);
...
function create_map() {
	...
	if (window.map) {
		...
		if (!cached_map && map_tiles.length && map.children)
			map.removeChildren(map.children.indexOf(map_tiles[0]), map.children.indexOf(map_tiles[map_tiles.length - 1])); // [06/03/20]
	...
	if (!tile_sprites[current_map])
		(tile_sprites[current_map] = []), (tile_textures[current_map] = []), (sprite_last[current_map] = []);
	...
	for (var i = 0; i < GEO.tiles.length; i++) {
		...
		if (!tile_sprites[current_map][i]) {
			...
			if (!cached_map)
				tile_sprites[current_map][i][sprite_last[current_map][i]] = new_map_tile(tile_textures[current_map][i]); // new - cache one in case it increases performance [03/09/16]
		}
	}
	...
	if (cached_map) {
		...
		for (var last_water_frame = 0; last_water_frame < 3; last_water_frame++) {
			rtextures[last_water_frame] = PIXI.RenderTexture.create(
				GEO.max_x - GEO.min_x,
				GEO.max_y - GEO.min_y,
			...
			for (var i = 0; i < GEO.placements.length; i++) {
				...
				if (tile[3] === undefined || tile[3] === null) {
					...
					if (sprite_last[current_map][tile[0]] >= tile_sprites[current_map][tile[0]].length)
						tile_sprites[current_map][tile[0]][sprite_last[current_map][tile[0]]] = new_map_tile(
							tile_textures[current_map][tile[0]],
						);
					...
					if (abs(((tile[3] - tile[1]) / w + 1) * ((tile[4] - tile[2]) / h + 1)) >= 20) {
						...
						if (tile_textures[current_map][tile[0]].length > 1)
							// new_map_tile logic
							texture =
								tile_textures[current_map][tile[0]][last_water_frame % tile_textures[current_map][tile[0]].length];
		...
		if (0) {
			(window.ftile = new PIXI.Sprite(textures.fog[0])),
				(ftile.x = 0),
				(ftile.y = 0),
				(ftile.parentGroup = sprite.displayGroup = fog_layer);
			(window.ftile2 = new PIXI.Sprite(textures.fog[1])),
				(ftile2.x = -500),
				(ftile2.y = 0),
				(ftile2.parentGroup = sprite.displayGroup = fog_layer);
			(window.ftile3 = new PIXI.Sprite(textures.fog[1])),
				(ftile3.x = 500),
				(ftile3.y = 0),
				(ftile3.parentGroup = sprite.displayGroup = fog_layer);
		...
		(G.maps[current_map].monsters || []).forEach(function (mdef) {
			if (mdef.boundary) {
				var e = empty_rect(
					mdef.boundary[0],
					mdef.boundary[1],
					mdef.boundary[2] - mdef.boundary[0],
					mdef.boundary[3] - mdef.boundary[1],
					2,
					0xfc5f39,
				);
				map.addChild(e);
			...
			if (mdef.rage) {
				var e = empty_rect(
					mdef.rage[0] - 3,
					mdef.rage[1] - 3,
					mdef.rage[2] - mdef.rage[0] + 6,
					mdef.rage[3] - mdef.rage[1] + 6,
					2,
					0x916bbd,
				);
				map.addChild(e);
...
function retile_the_map() {
	...
	if (
		!(map.last_max_y == undefined || abs(map.last_max_y - max_y) >= border || abs(map.last_max_x - max_x) >= border)
	) {
		if (last_water_frame != water_frame()) {
			last_water_frame = water_frame();
	...
	for (var i = 0; i < map_tiles.length; i++) {
		...
		if (
			...
			entity.x > max_x ||
			entity.y > max_y ||
			entity.x + entity.width < min_x ||
			entity.y + entity.height < min_y
		) {
	...
	if (GEO["default"] && !mdraw_tiling_sprites) {
		...
		for (var x = min_x; x <= max_x + 10; x += def[3])
			for (var y = min_y; y <= max_y + 10; y += def[4]) {
				...
				if (sprite_last[current_map][GEO["default"]] >= tile_sprites[current_map][GEO["default"]].length)
					(tile_sprites[current_map][GEO["default"]][sprite_last[current_map][GEO["default"]]] = new_map_tile(
		...
		if (!window.default_tiling)
			default_tiling = new PIXI.extras.TilingSprite(
				...
				floor((max_x - min_x) / 32) * 32 + 32,
				floor((max_y - min_y) / 32) * 32 + 32,
			);
	...
	for (var i = 0; i < GEO.placements.length; i++) {
		...
		if (is_nun(tile[3])) {
			...
			if (
				...
				(mdraw_tiling_sprites && abs(((tile[3] - tile[1]) / w + 1) * ((tile[4] - tile[2]) / h + 1)) < 8)
			) {
				for (var x = tile[1]; x <= tile[3]; x += w) {
				...
				if (!window["defP" + current_map + i])
					window["defP" + current_map + i] = new PIXI.extras.TilingSprite(
						tile_textures[current_map][tile[0]][0],
						tile[3] - tile[1] + w,
						tile[4] - tile[2] + h,
					);
...
function calculate_fps() {
	...
	if (mssince(last_count) >= frequency)
		(last_count = new Date()), (fps = (frames - last_frame) * (1000.0 / frequency)), (last_frame = frames);
...
function load_game(c) {
	loader.load(function (loader, resources) {
		...
		if (mode_nearest)
			for (file in PIXI.utils.BaseTextureCache) PIXI.utils.BaseTextureCache[file].scaleMode = PIXI.SCALE_MODES.NEAREST; // great find - prevents tile gaps and upper shadows / similar imperfections [17/06/16]
		...
		for (name in G.sprites) {
			...
			if (in_arr(s_def.type, ["v_animation", "head", "hair", "hat", "s_wings", "face", "makeup", "beard"]))
				(col_num = 1), (s_type = s_def.type);
		...
		G.positions.textures.forEach(function (name) {
			...
			textures[name] = new PIXI.Texture(
				PIXI.utils.BaseTextureCache[G.tilesets[position[0]].file],
				new PIXI.Rectangle(position[1], position[2], position[3], position[4]),
			);
		});
